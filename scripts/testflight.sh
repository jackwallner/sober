...
#
# If you already have build/Sober.xcarchive:
...
if [[ -f "$PROJECT_YML" ]]; then
    ...
    if [[ -n "$CURRENT_BUILD" && "$CURRENT_BUILD" =~ ^[0-9]+$ ]]; then
        ...
        git add "$PROJECT_YML" \
            && git commit -m "chore(sober): bump build $CURRENT_BUILD -> $NEW_BUILD" || true
...
ARCHIVE="$ROOT/build/Sober.xcarchive"
rm -rf "$ARCHIVE"

echo "==> Archive (Release)"
xcodebuild -project Sober.xcodeproj \
  -scheme Sober \
